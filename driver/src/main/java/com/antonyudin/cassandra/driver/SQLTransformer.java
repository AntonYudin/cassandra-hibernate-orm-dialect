...
public class SQLTransformer implements java.io.Serializable {
	...
	public String transform(final String originalSQLNoTrim) {
		...
		if (originalSQL.startsWith("select")) {
			...
			final int indexOfSpace0 = originalSQL.indexOf(" ", indexOfFrom + patternFrom.length());
			...
			final String tableName = originalSQL.substring(indexOfFrom + patternFrom.length(), indexOfSpace0);
			final String aliasName = originalSQL.substring(indexOfSpace0 + 1, indexOfSpace1).trim();
